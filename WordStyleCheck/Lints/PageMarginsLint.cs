    ...
    public void Run(ILintContext ctx)
    {
        foreach (var section in ctx.Document.AllSections)
        {
            ...
            if (!margins.CloseTo(target))
            {
                ...
                {
                    ctx.AddMessage(new LintDiagnostic("IncorrectPageMargins", DiagnosticType.FormattingError,
                        new SectionDiagnosticContext(section))
                    {
                        Parameters = new()
                        {
                            ...
                            ["ExpectedBottomCm"] =
                                Utils.TwipsToCm(target.Bottom).ToString(CultureInfo.InvariantCulture),
                            ...
                            ["ExpectedHeaderCm"] =
                                Utils.TwipsToCm(target.Header).ToString(CultureInfo.InvariantCulture),
                            ["ExpectedFooterCm"] =
                                Utils.TwipsToCm(target.Footer).ToString(CultureInfo.InvariantCulture),
                            ["ExpectedGutterCm"] =
                                Utils.TwipsToCm(target.Gutter).ToString(CultureInfo.InvariantCulture),
